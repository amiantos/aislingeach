...
class GenerationTracker {
    ...
    func saveFinishedGenerations(request: HordeRequest) {
        ...
        HordeV2API.getImageAsyncStatus(_id: requestId, clientAgent: hordeClientAgent()) { [self] data, error in
            if let data = data {
                Log.debug("\(data)")
                if data.done ?? false, let generations = data.generations, !generations.isEmpty {
                    var count = 0
                    generations.forEach { generation in
                        if let urlString = generation.img,
                           let imageUrl = URL(string: urlString)
                        {
                            DispatchQueue.global().async {
                                if let imageData = try? Data(contentsOf: imageUrl) {
                                    DispatchQueue.main.async {
                                        ImageDatabase.standard.saveImage(id: generation._id!, requestId: requestId, image: imageData, request: request, response: generation, completion: { generatedImage in
                                            if let image = generatedImage {
                                                Log.info("\(requestId) - Saved Image ID \(image.uuid!)")
                                                count += 1
                    ...
                    Log.error("No generations found within the request...?")
                    generationsSaved = generationsSaved.filter { $0 != requestId }
                }
            } else if let error = error {
                Log.error("\(requestId) - Encountered error trying to fetch image: \(error)")
                generationsSaved = generationsSaved.filter { $0 != requestId }
            }
        }
    }
