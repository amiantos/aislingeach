...
class ViewController: UIViewController {
    ...
    @IBAction func generateButtonPressed(_ sender: UIButton) {
        ...
        V2API.postImageAsyncGenerate(body: GenerationInputStable(prompt: generationText), apikey: Preferences.standard.apiKey, clientAgent: UAString()) { data, error in
...
extension ViewController {
    ...
    @objc func checkCurrentGenerationStatus() {
        ...
        V2API.getImageAsyncCheck(_id: generationIdentifier, clientAgent: UAString()) { data, error in
            if let data = data {
                Log.debug("\(data)")
    ...
    func getFinishedImageAndDisplay() {
        ...
        V2API.getImageAsyncStatus(_id: generationIdentifier, clientAgent: UAString()) { [self] data, error in
            if let data = data {
                Log.debug("\(data)")
                if data.finished == 1 {
